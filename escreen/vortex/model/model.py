...
class ParallelGatedConvBlock(nn.Module):
    def __init__(self, config, layer_idx, hyena_filter_groups=None, fir_inner_filter_length=None) -> None:
        ...
        self.projections = nn.Linear(config.hidden_size, 3 * config.hidden_size, bias=config.qkv_proj_bias).to(dtype=dtype)
        #TELinear(
        #    config.hidden_size,
...
def get_block(config, layer_idx, flash_fft=None):
    if layer_idx in config.attn_layer_idxs: # 这个对应MHA
        return AttentionBlock(config, layer_idx)
    elif layer_idx in config.hcl_layer_idxs: # 这个对应HyenaLI
        block = ParallelGatedConvBlock(config, layer_idx)
        if config.get("use_flashfft", "False"):
            block.filter.fftconv_fn = flash_fft
        return block
    elif layer_idx in config.hcm_layer_idxs: # 这个对应HyenaMR
        ...
        return block
    elif layer_idx in config.hcs_layer_idxs: # 这个对应hyenaSE
        block = ParallelGatedConvBlock(
            config,
            layer_idx,
